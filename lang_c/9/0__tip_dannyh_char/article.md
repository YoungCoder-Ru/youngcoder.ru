...
Кстати, иногда ключевые слова `signed` и `unsigned` называют =модификаторами типа=. Но судя по стандарту языка следует говорить о трёх различных типах данных: `signed char`, `unsigned char` и простой `char`, диапазон значений которого обязательно совпадают либо с одним из них.
