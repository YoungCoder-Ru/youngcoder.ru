...
Если, например, мы попробуем вывести значение переменной `i` вне цикла, то программа даже не скомпилируется. Компилятор будет ругаться о том, что мы пытаемся использовать необъявленную переменную `i`. Убедитесь в этом самостоятельно, скомпилировав программу из Листинга 15.
