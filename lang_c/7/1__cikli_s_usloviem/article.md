...
Цикл `for`, изученный нами в предыдущем уроке, очень удобно применять, когда заранее известно количество итераций или когда это количество можно легко вычислить. Но бывают ситуации, когда мы не знаем заранее, сколько раз должен выполниться цикл. Например, когда количество повторений зависит от действий пользователя или от данных, с которыми работает программа.
...
Эта программа работает, но работает лишь один раз, а мы бы хотели, чтобы она бросала кубик до тех пор, пока мы сами не решим закончить игру и не введём число `0`. Тут-то мы и сталкиваемся с проблемой. Понятно, что заранее узнать, когда игра закончится, невозможно. А значит использовать цикл `for` напрямую не получится. Да и не нужно, ведь есть более элегантный выход -- использование других циклических конструкций. Например, цикла `while`.
...
Обратите внимание, что в цикле `while` (как и в цикле `for`) прежде чем выполнить тело цикла сперва проверяется условие (значение управляющего выражения). Из-за этого `while` также относится к циклам с предусловием.  
...
Вроде бы всё хорошо, но при внимательном рассмотрении кода из Листинга 3 можно заметить, что мы повторяем один и тот же фрагмент кода
...
% **Важно!**
Всегда проверяйте, что в теле цикла есть код, который изменяет переменные, от которых зависит управляющее выражение.
...
**Шаг 4.** Возвращаемся к проверке условия цикла `while`. В переменной `control` по-прежнему хранится значение `2` (оно не изменилось, т.к. мы не выполняли `scanf`). Управляющее выражение `2 != 0` снова принимает значение `ИСТИНА`;
...
Ещё раз обращаю внимание, что условие проверяется **после** того, как будет выполнено тело цикла. То есть тело цикла будет выполнено минимум один раз, даже если условие изначально ложно.
...
В общем-то очень похоже на предыдущий код. Правда, пришлось вернуть ветку `case 0:` в инструкцию `switch`. Иначе из-за того, что проверка производится после выполнения тела цикла, программа работала бы некорректно. Если бы мы ввели нуль, то выводилось бы сообщение об ошибке. В программе с циклом `while` подобной ситуации быть не могло, так как равенство нулю проверялось в условии цикла. При вводе нуля условие стало бы ложью и тело цикла бы не выполнилось.
...
> **Пример 1.** Программа должна вывести на экран 5 раз строку "hello, friend..."
...
% **Важно!** В теле цикла можно использовать **любые инструкции** языка Си: вызовы функций, условия, любые виды циклов и т.д.
...
int main(void)
{
        ...
        for(scanf("%d", &number); number != 0; scanf("%d", &number)) {
                // очень плохой заголовок получился =)
                printf("You entered: %d\n", number);
        }
...
На мой взгляд, наиболее понятным в этой ситуации выглядит цикл `while`.
