...
Вы, возможно, уже подумали, что инструкция `continue` гораздо хуже, чем инструкция `break`, но это, конечно, абсолютно неверно. Полученный результат лишь показывает, что `continue` в данной программе используется не по назначению и здесь более уместно использовать инструкцию `break`.
