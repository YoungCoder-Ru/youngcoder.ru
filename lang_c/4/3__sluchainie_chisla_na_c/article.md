...
% **Важно!**
Слово случайное взято в кавычки намеренно. Подробнее об этом читайте в дополнительных материалах к этому уроку.
...
Единичку прибавлять обязательно, иначе само число `K` никогда не будет сгенерировано.
...
Как видим, проблема у нас заключается в правой границе отрезка, она сдвинута вправо на `80` единиц. Это как раз число `80`, которое мы прибавляем, чтобы сдвинуть нижнюю (левую) границу отрезка. Но добавление числа сдвигает обе границы и нижнюю (левую) и верхнюю (правую). Давайте наведём порядок и сдвинем правую границу назад: `80 + rand()%(101 - 80)`. Теперь всё должно работать, как надо.
