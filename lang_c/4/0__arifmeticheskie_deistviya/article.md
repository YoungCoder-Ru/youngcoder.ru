...
У вас не получится. Компилятор начнёт ругаться на строку `rem_real = c % d;`. Дело в том, что оператор `%` может работать только со значениями целого типа, а мы пытаемся ему подсунуть вещественные числа `10.0` и `3.0`. Поэтому он и ругается: `"Operands of '%' have incompatible types: 'double' and 'double'."`
