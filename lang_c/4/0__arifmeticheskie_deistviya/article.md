...
Вы можете спросить: почему в листинге 3 мы приводили к типу `float` именно числитель? Всё дело в том, что в процессе выполнения, если типы переменных разные, программа автоматически приводит _менее точные типы к более точным_. Например, если в делении будут типы `int/float`, то программа приведёт тип `int` к `float`, чтобы получилось `float/float`. Если будет дробь `float/double`, то станет `double/double`.

Такой способ автоматического приведения типов (без участия прямого участия программиста) называется =неявным преобразованием типа=. Таким образом, листинги ниже будут эквивалентны листингу 3.

Листинг 5. Возможная вариация листинга 3.
```c
#include <stdio.h>
...
```
Листинг 6. Ещё одна вариация листинга 3.
```c
#include <stdio.h>
...
А что будет, если выражение будет более сложным? Например, у нас есть такое выражение: `A/(B/C)/D`, при этом все переменные целочисленные. В таком случае, необходимо применять явное приведение к тем переменным, которые первые участвуют в вычислении. В примере это `B` или `C`.

% **Важно!**
При приведении типов обращайте пристальное внимание к какой переменной в выражении вы его применяете.
