...
Мы используем имя функции, чтобы обратиться к функции (вызвать функцию). Каждая функция должна иметь своё уникальное имя. В имени функции можно использовать: английские буквы, цифры и символ `_`. Причём имя не должно начинаться с цифры.
...
А что же делать, если нашей функции не нужны параметры? В таких случаях, как мы знаем с самого первого урока, вместо списка параметров в круглых скобках нужно записать ключевое слово `void`. 
...
Давайте перепишем Листинг 4 с использованием прототипа функции `max2`.
...
- **Аргументы (фактические параметры) функции** -- конкретные значения, которые реально передаются в функцию при её вызове.
%
...
% **Важно!** Если в определении функции указан тип возвращаемого значения, то функции обязательно должна возвращать какое-либо значение с помощью инструкции `return`. Если в какой-то ветке выполнения функции `return` не сработает по тем или иным причинам, то мы получим =неопредлённое поведение (англ. undefined behavior, UB)= 

Ранее мы упоминали, что существуют функции, которые не должны возвращать никакого значения (у них вместо типа возвращаемого значения записано слово `void`). Иногда их называют `void`-функциями или =процедурами=.  Как же быть с ними? 
...
Допустим, что после запуска программы мы ввели `5 11`. Функция `scanf` запишет значения `5` и `11` в переменные `x` и `y` соответственно.
...
% **Важно!** Обратите внимание, что все переменные, объявленные внутри функции являются **локальными** (включая и переменные из параметров). Они доступны только внутри этой функции. Говоря на профессиональном жаргоне: область видимости этих переменных ограничена телом функции, а время жизни -- временем работы функции.

**Шаг 5.** Вызов функции `max2(5, 11)` заменяется значением, которое вернула функция `max2`. 
...
Компилятор, конечно, тоже за этим следит. В некоторых простых случаях, если он заметит расхождение, то можеть выдать предупреждение, а иногда может выполнить неявное приведение типов. При этом, как обычно, возможна потеря точности. Если же приведение типа выполнить невозможно, то вы получите ошибку компиляции. 
...
Для `void`-функций пункт `5` не выполняется, а 4 пункт состоит в удалении локальных переменных и передаче управления вызывающей программе. 
...
Во-первых, посмотрите как ловко мы переиспользовали уже готовую функцию `max2`, чтобы написать функцию `max3`.
