...
В этой заметке на примере программы «Игральный кубик» из прошлых уроков я бы хотел проиллюстрировать некоторые возможности использования функций.
...
Для начала хорошо бы выделить части кода, которые можно вынести в отдельные функции. Подобную операцию иногда называют =декомпозицей=, т.е. разделением одной большой программы на ряд небольших вспомогательных программ. Правило "Divide and conquer" работает и в программировании.

Во-первых, сразу бросаются в глаза два куска кода, где есть последовательные вызовы функции `printf`:
1. вывод приветственного экрана;
...
Тут ничего особо сложного. Я буквально просто скопировал код и внёс небольшие косметические изменения. В функции `show_intro_screen` я использовал подход, который называется string literal concatenation. Я описывал его в дополнительных материалах к уроку про `printf`.

В функции `show_exit_screen` пришлось заменить имена локальных переменных на те, что использованы в заголовке функции, хотя это можно было и не делать.

Давайте объединим наши наработки из Листингов 2 и 3 в единый код.

Листинг 4.  «Игральный кубик» v.3.1
```c
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
Главная функция `main` стала гораздо чище.
...
% **Рекомендация:** функция должна решать одну конкретную задачу. И желательно, решать её очень хорошо.

Если бы я добавил в эту функцию ещё и вывод строки на экран, то это уже было бы два действия. А когда функция выполняет два действия, то ей сложно придумать хорошее имя. Не называть же её `generate_dice_value_and_print_result`? 
...
% **Важно!** нет никаких золотых стандартов и правил, как правильно разбивать программу на подпрограммы. Единственный способ научиться правильно это делать -- постоянно тренироваться, держа в уме главную цель декомпозиции: упростить код!

Дополнительно, я создал отдельную переменную `DICE_SIZE` в функции `main` в которой будет храниться количество граней на нашей игральной кости. 
...
Все переменные, которые мы объявляем внутри функций являются локальными. Они видны только в пределах той функции, в которой они объявлены. Но, вообще говоря, мы могли бы объявить переменную вне любых функций, т.е. даже вне функции `main`. Такие переменные называются =глобальными переменными=.

У них две основные особенности:
1. глобальные переменные инициализируются значением `0` по умолчанию, если они не инициализированы каким-либо другим значением.
2. глобальные переменные доступны в любой функции этого файла и это, с одной стороны удобно, а с другой -- довольно опасно.

Давайте напишем очередную версию программы  «Игральный кубик», но с использованием глобальных переменных. Глобальными сделаем переменные `DICE_SIZE`, `total_runs` и `sum_results`.
...
Что изменилось? 
Во-первых, в функции больше не надо передавать параметры. Во всех прототипах вместо списка параметров указано ключевое слово `void`. Передавать параметры между функциями больше не нужно, т.к. все эти переменные являются глобальными и доступны всем функциям в этом файле.

Во-вторых, т.к. глобальные переменные по умолчанию инициализируются нулевыми значениями (в отличие от локальных), то при объявлении переменных `total_runs` и `sum_results` опущена инициализация нулями.
...
Что ж, давайте защитим нашу переменную `DICE_SIZE` от случайного изменения какими-либо функциями.

Листинг 9: Программа «Игральный кубик» v.3.4 с защитой глобальной переменной `DICE_SIZE`
```c
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
Теперь переменная `DICE_SIZE` под защитой квалификатора `const`.
...
Поэтому на основе Листинга 6 я сделаю новую версию программы: без глобальных переменных, с "защитой" через `const` переменной `DICE_SIZE` и параметров, которые функции только читают. Кроме того, я изменю названия некоторых переменных на более удачные варианты.
...
void show_intro_screen(void);
void show_exit_screen(const int runs, const int sum);
...
НО, во-первых, теперь код имеет некоторое подобие внутренней структуры. Он написан не одним сплошным куском, а состоит из отдельных небольших функций, каждая из которых решает свою локальную небольшую задачу. Две из них можно назвать "интерфейсными", а одну "игровой". 
...
% **Когда имеет смысл выносить код в функцию:**
- если фрагмент кода **повторяется** в нескольких местах программы; 
- если часть кода выглядит как отдельный законченный шаг/подзадача алгоритма;
